...
void inject_after(dni::Graph* g, int after, int n, int interval)
{
        ...
        std::vector<dni::IP> cidr_merged_sip = cidr_merge(ip_map);

        float_t number_ratio_min = 0.1;
        float_t number_ratio_max = 0.6;
        std::vector<std::string> number_stat_type = {
            "centralize", "regular", "random", "void"};

        float_t protocol_ratio_min = 0.1;
        float_t protocol_ratio_max = 0.6;
        ...
        for (int i = 0; i < n; i++)
        {
                ...
                g->AddDatumToInputStream("cidr_merged_sip", dni::Datum(cidr_merged_sip));
...
int main()
{
        ...
        auto ret =
            g->GetResult<std::unordered_map<std::string, dni::SIPBaseMergeStats>>(out);
