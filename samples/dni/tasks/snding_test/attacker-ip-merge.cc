...
int main()
{
    ...
    std::string logFileName = "attacker-ip-merge.log";  
    ...
    const std::string& proto = relatedpath + "testdata/attacker_ip_merge.pbtxt";
    ...
    for (size_t i = 0; i < test_datasets.size(); ++i) {
        ...
        for (const auto& ip : ret6.attackerIPs) {
          ...
          outFile << "CIDR IP: " << ip.ip << ", Length: " << ip.len << std::endl;
        }

        if (ret6.containRandomAttack) {
            outFile << "containRandomAttack: " << ret6.containRandomAttack << std::endl << "Random IP: ";
            int j = 1;
            for (uint32_t ip : ret6.randomIPs) {
              ...
              outFile << ip << " , ";
              j++;
              if (j == 100) { outFile << "......"; break; }
            }
            outFile << std::endl;
        }else {
            outFile << "containRandomAttack: " << ret6.containRandomAttack << std::endl;
        }     
