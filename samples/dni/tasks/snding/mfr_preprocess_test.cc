    ...
    struct ether_header* eth_header, uint16_t* h_proto, uint64_t* l3_off)
{
        ...
        *h_proto = eth_header->ether_type;
        ...
        for (i = 0; i < 2; i++)
        {
                spdlog::info("[{}a], proto:0x{:X}", i, ntohs(*h_proto));
                if (ntohs(*h_proto) != ETH_P_8021Q && ntohs(*h_proto) != ETH_P_8021AD)
                {
                        spdlog::info("neither 802.1q nor 802.1ad");
                        break;
                }
                *l3_off += VLAN_HLEN;
                *h_proto = *(__be16*) ((u_char*) eth_header + (*l3_off) - ETHER_TYPE_LEN);
                spdlog::info("[{}b], encapsulated proto:0x{:X}", i, ntohs(*h_proto));
        }
...
void parse_snd_features(u_char* args, const struct pcap_pkthdr* hdr, const u_char* body)
{
        ...
        spdlog::info("caplen: {}, len: {}", hdr->caplen, hdr->len);
        ...
        spdlog::info("ip option len: {}", ip_option_len);
        ...
        case IPPROTO_TCP: {
                ...
                spdlog::info("src_port = {}", ntohs(tcp->source));
                spdlog::info("dst_port = {}", ntohs(tcp->dest));
                ...
                spdlog::info("tcp option len: {}", opt_len);
                ...
                spdlog::info("tcp data len: {}", data_len);
        ...
        case IPPROTO_UDP: {
                ...
                spdlog::info("udp data len: {}", data_len);
        ...
        case IPPROTO_ICMP: {
                ...
                spdlog::info("icmp data len: {}", data_len);
        ...
        default: {
                ...
                spdlog::info("default protocol data len: {}", data_len);
...
void get_mfr_bytes(
    ...
    int batch_size = 64)
{
        spdlog::info("loading pcap file {} ...", pcap);
        ...
        spdlog::info("analysis_mfr_data size: {}", analysis_mfr_data_size);
        ...
        spdlog::info("real_batch_size size: {}", real_batch_size);
...
int main()
{
        ...
        // const std::string& pcap =
        // "/home/zhouxu/works/2024/50_dni/pcap/mfr/orig/tcp.pcap";
        const std::string& pcap =
        ...
        //         // std::vector<mfr_input_tensor> model_np_list;

        //         float* model_np_list =
        //             (float*) malloc(batch_size * 1 * 40 * 40 * sizeof(float));
