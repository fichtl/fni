...
void inject_after1(dni::Graph* g, int after, int n, int interval)
{
        ...
        for (int i = 0; i < n; i++)
        {
                ...
                g->AddDatumToInputStream("host_nic_name", dni::Datum("target1#eno3"s));
...
int main()
{
        ...
        const std::string& proto =
            "samples/dni/tasks/snding/testdata/sip_base_merge_dedup.pbtxt";
        ...
        auto ret2 = g->GetResult<std::unordered_map<
            std::string, std::unordered_map<std::string, std::string>>>(out2);
