...
std::string inject_after(dni::Graph* g, int after, int n, int interval, const std::string& hostName, const std::string& nicName, const std::string& tableName, const std::string& tableName1, const std::string& tableName2)
{
...
anomalyResult anomaly_detect(std::string hostName, std::string nicName, std::string tableName, std::string tableName1, std::string tableName2, std::string protoPath1) {
    ...
    g->PrepareForRun();
    std::string pcapNameStr = inject_after(g, 0, 1, 0, hostName, nicName, tableName, tableName1, tableName2);
    g->RunOnce();
    g->Wait();
    ...
    spdlog::info("G1out pcapName result is: {}", pcapNameStr);
    g->Finish();

    anomalyResult anomaly_result;
    anomaly_result.abnormal_number = ret3;
    anomaly_result.pcapName = pcapNameStr;
...
int main() {
    ...
    anomalyResult anomaly_result = anomaly_detect(hostName, nicName, tableName, tableName1, tableName2, protoPath1);
    ...
    if (anomaly_result.abnormal_number > 0) {
        ...
        g->RunOnce();
        g->Wait();
        auto ret = g->GetResult<double_t>(attack_result);
        spdlog::info("G2out {} result is: {}", attack_result, ret);
