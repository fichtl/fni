...
void split_string(
    const std::string& str, const std::string& split, std::vector<std::string>& res)
{
...
{
        auto client =
            influxdb::InfluxDBFactory::Get("http://172.17.17.33:8086?db=sccmmaster");
        ...
        auto query_ret = client->execute("select netif,host_name from baseinfo Group by "
                                         "host_id order by time desc limit 1");
        ...
        for (int i = 0; i < series.size(); i++)
        {
                ...
                for (auto&& if_ip : ips_in_one_dev)
                {
                        ...
                        if (std::regex_match(ip, ipv4_pattern))
                        {
                                ...
                                unsigned int ip_num = (std::stoul(ip_segs[0]) << 24) +
                                ...
                                host_nic_names.push_back(host_name + "#" + nic_name);
                        }
                }
