...
__attribute__((unused)) const char cert[] = "/var/dni_grpc/server_certs/cert.pem";
const char certKey[] = "/var/dni_grpc/server_certs/private.key";
const char certChain[] = "/var/dni_grpc/server_certs/full_chain.pem";
const char rootCert[] = "/var/dni_grpc/server_certs/myssl_root.cer";
...
void RunServer()
{
        ...
        DNIServiceImpl dni_srv;
        const char* backend = getenv("GRPC_DNI_SERVER_BACKEND");
        std::string endpoint(backend ? backend : "");
        if (!endpoint.empty())
        {
                dni_srv.setChannel(Connection::getChannel());
        }
        builder.RegisterService(&dni_srv);
...
int main(__attribute__((unused)) int argc, char** argv)
{
        ...
        RunServer();
        SPDLOG_INFO("DNI_SERVER gRPC C++ Server is stopping");
