...
#include "samples/ddos/realtime_A100/common/rb_data_parse.h"
...
anomalyResult g1_anomaly_detect(dni::Graph* g, dni::RBDataHeader& rb_header)
{
        ...
        SPDLOG_DEBUG("{} result is: {} ---> {}", out, ret, (int(ret + 0.5)));

        anomalyResult anomaly_result;
        anomaly_result.abnormal_number = (int(ret + 0.5));
        anomaly_result.incrbytesRecv = 0;
        anomaly_result.inMbps_score = 0;
        anomaly_result.inKpps_score = 0;
        anomaly_result.incrpacketRecv = 0;
