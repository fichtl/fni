...
namespace dni {
        ...
        public:
                ...
                int Process(TaskContext* ctx) override
                {
                        ...
                        SPDLOG_DEBUG(
                            "Task {}: Consume side data features: {}", name_, features_d);

                        auto features_opt =
                            features_d.Consume<std::vector<std::string>>();
                        if (!features_opt)
                        {
                                SPDLOG_WARN(
                                    "Task {}: Consume() returns NULL, wait for "
                        ...
                        SPDLOG_DEBUG("Task {}: features: {}", name_, features);

                        SPDLOG_DEBUG(
                            "Task {}: pcap_packets: {}",
                            name_,
                            pcap_packets.back()["SIP"]);

                        // count, need to spilit to {"SIP", "SPort", "DPort", "Protocol",
                        // "Length"}
                        std::vector<std::unordered_map<uint32_t, int>>
                            ret;   // map: key-number: count
