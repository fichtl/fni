 ...
 */
int split_range(uint32_t ip, int len, uint32_t lo, uint32_t hi, std::vector<CIDR>& CIDRs)
{
        ...
        {
                if (split_range(lower_half, len, lo, broadcast(lower_half, len), CIDRs) ==
                    -1)
                        return -1;
...
std::vector<CIDR> iprange_to_cidrs(CIDR from, CIDR to)
{
        ...
        {
                SPDLOG_ERROR(
                    "len bigger than 32, from.len: {}, to.len: {}", from.len, to.len);
