...
template <>
struct formatter<dni::snding::AttackerIPMergeResult>: formatter<std::string_view> {
        auto format(
            const dni::snding::AttackerIPMergeResult& result, format_context& ctx) const
        {
                return format_to(
                    ctx.out(),
                    "attackerIPs: {}\n, containRandomAttack: {}\n, randomIPs: "
                    "{}\n",
                    fmt::join(result.attackerIPs, ","), result.containRandomAttack,
                    fmt::join(result.randomIPs, ","));
        }
};
...
template <>
struct formatter<dni::snding::SIPBaseMergeNumberStats>: formatter<std::string_view> {
        auto format(
            const dni::snding::SIPBaseMergeNumberStats& stats, format_context& ctx) const
        {
                return format_to(
...
struct formatter<std::pair<const int, std::string>> {
        ...
        auto format(const std::pair<const int, std::string>& fp, FormatContext& ctx) const
        {
...
template <>
struct formatter<dni::snding::SIPBaseMergeProtoStats>: formatter<std::string_view> {
        auto format(
            const dni::snding::SIPBaseMergeProtoStats& stats, format_context& ctx) const
        {
                auto j =
                    fmt::join(stats.stat_types.begin(), stats.stat_types.end(), ", ");
...
struct formatter<dni::snding::SIPBaseMergeStats>: formatter<std::string_view> {
        auto format(
            const dni::snding::SIPBaseMergeStats& stats, format_context& ctx) const
        {
                return format_to(
                    ctx.out(),
                    "hostNicSign: {}\n, srcIP: {}\n, dstIP: {}\n, srcPort: {}\n, "
