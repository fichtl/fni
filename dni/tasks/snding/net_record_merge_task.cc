...
public:
        ...
        int Process(TaskContext* ctx) override
        {
                ...
                std::vector<std::unordered_map<std::string, snding::SIPBaseMergeStats>>
                    stats_in_all_net;
                for (size_t i = 0; i < nic_size; i++)
                {
                        ...
                        stats_in_all_net.emplace_back(std::move(sip_base_merge_stats));
                }
                // match key: srcip-cidr:dstip/32, format is uint32/len:uint32/len
                ...
                for (auto&& nic_stats : stats_in_all_net)
                {
                        ...
                        for (auto&& stat : nic_stats)
                        {
                                ...
                                {
                                        // downstream node will use "RANDOM-dstip" or
                                        // "srcip-dstip" to match attack link. all
                                        // "RANDOM-dstip" matched host as an unique attack
                                        // link.
                                        auto key = stat.first + ":" +
                                        ...
                                        attack_link.insert(stat.second.hostNicSign);
                                }
                        }
                }

                SPDLOG_DEBUG("{}: after calculation: {}", name_, hosts_in_attack_link);

                ctx->Outputs()[0].AddDatum(Datum(std::move(hosts_in_attack_link)));
