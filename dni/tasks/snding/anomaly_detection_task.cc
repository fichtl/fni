        ...
        public:
                ...
                int Process(TaskContext* ctx) override
                {
                        ...
                        for (int i = 0; i < ctx->Inputs().size(); i++)
                        {
                                ...
                                SPDLOG_DEBUG(
                                    "Task {}: Consume Datum: {}", name_, judge_d);
                                auto judge_opt = judge_d.Consume<double_t>();
                                if (!judge_opt)
                                {
                                        SPDLOG_WARN(
                                            "Task {}: Consume() judge returns "
                                            "NULL, wait for "
                                            "input ...",
                                            name_);
                                }
                                auto judge = *(judge_opt.value());
                                SPDLOG_DEBUG("Task {}: judge: {}", name_, judge);
                        ...
                        SPDLOG_DEBUG(
                            "Task {}: after calculation: {}", name_, abnormal_type);
